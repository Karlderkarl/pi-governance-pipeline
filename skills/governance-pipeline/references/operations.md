...
| `--auto-merge` | off | Confirmed at startup; also grants implementer trust (`--approve` / `bypassPermissions`) by itself. Branch merging is not implemented |
...
| `PIPELINE_UNATTENDED` | — | Exported to child processes once the startup gate has passed; the guard reads it. An inherited value is dropped by the engine with a warning: trust comes from confirmed `--unattended` or `--auto-merge`, never from the caller's environment |
