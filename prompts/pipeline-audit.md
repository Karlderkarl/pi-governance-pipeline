...
5. Deterministic gates (lint, tests — a clean-code check folds into the lint command, there is no separate slot) run before any model-based review, and the script says so at start when neither is configured, rather than satisfying the invariant with nothing to run.
6. The master review runs on every attempt, sees the original reviewer JSON, and cannot approve when the deterministic gate blocked. Its verdict is parsed fail-closed, and among several parseable decisions the strictest wins.
...
19. Role toolset: every role passes `--no-session`; `review.*` passes `-nc`, `-t read,grep,find,ls` and `--no-approve`; `controller` and `master_review` pass `--no-tools` once the diff is truncated per file with an omitted-path manifest. A `review.*` role must never receive `--approve`: `-nc` alone leaves the trust-gated `.pi/APPEND_SYSTEM.md` in reach.
