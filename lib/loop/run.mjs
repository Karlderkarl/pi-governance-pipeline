...
import { quoteDecisionMarkers } from "../contract/parse.mjs";
...
import { GUARD_OFF_WARNING, guardDisabled } from "../guard/patterns.mjs";
import { preservePaths } from "../integrity/governance-paths.mjs";
import { controlPaths, guardControl } from "../integrity/control.mjs";
import { scanDecisionMarkers } from "../integrity/markers.mjs";
...
import { findingsToProse, followupsToProse } from "../review/findings-prose.mjs";
import { allDroppedGate, runGate } from "../review/gate.mjs";
import { parseMasterDecision } from "../review/master-decision.mjs";
import { KNOWN_SEVERITY, UNPROCESSED_SEVERITY, checkReviewerText, extractJson, severityOf } from "../review/reviewer-output.mjs";
import { attemptsOf, budgetOf, escalate, initState, loadState, readAllStates, recordAttempt, registerSplit, setContributors, setIssueStatus, setReviewPause } from "../state/store.mjs";
import { acquireRunLock } from "../state/lock.mjs";
import { git, hasGitBinary, hasHead, isGitWorkTree, runShell } from "../util/exec.mjs";
import { attemptTag, excerpt, headTail, intEnv, isoDate, runIdNow, sanitizeIssueId, sliceUtf8, tailLines } from "../util/text.mjs";
...
export const USAGE = `auto-develop — the governance-driven pipeline (pi-governance-pipeline).
  ...
  --auto-merge [--yes]                adaptation point: parsed and confirmed, not implemented;
                                      grants no privileges — that is --unattended
...
export async function runPipeline({ root, flags, env = process.env, stdout = process.stdout, stderr = process.stderr }) {
	...
	try {
		...
		if (error?.die || error?.code === "PIPELINE_CONTROL") {
...
async function run({ root, flags, env, stdout, stderr }) {
	...
	const number = (name, fallback, options = {}) => intEnv(env, name, fallback, { ...options, warn });
	...
	ctx.diffMaxBytes = number("DIFF_MAX_BYTES", 524288);
	ctx.binaryReviews = {};
	if (env.BINARY_REVIEW_FILE) {
		try {
			ctx.binaryReviewFile = realpathSync.native(resolve(root, env.BINARY_REVIEW_FILE));
			const rel = relative(realpathSync.native(root), ctx.binaryReviewFile);
			if (!(rel === ".." || rel.startsWith(`..${sep}`) || isAbsolute(rel))) throw new Error("must live outside the working tree");
			const receipt = JSON.parse(readFileSync(ctx.binaryReviewFile, "utf8"));
			if (receipt.version !== 1 || receipt.approved !== true || !receipt.files || typeof receipt.files !== "object" || Array.isArray(receipt.files) || Object.values(receipt.files).some((v) => typeof v !== "string" || !/^[a-f0-9]{64}$/.test(v))) throw new Error("expected version 1, approved: true and files mapping paths to SHA-256 hashes");
			ctx.binaryReviews = receipt.files;
		} catch (error) { throw new Die(`BINARY_REVIEW_FILE: ${error.message}`); }
	}
	ctx.reviewersMaxBytes = number("REVIEWERS_MAX_BYTES", 65536);
	ctx.exclusionsMaxLines = number("EXCLUSIONS_MAX_LINES", 200);
	ctx.blockerHistoryMax = number("BLOCKER_HISTORY_MAX", 5);
	ctx.blockerHistoryMaxBytes = number("BLOCKER_HISTORY_MAX_BYTES", 16384);
	ctx.promptKeepRuns = number("PROMPT_KEEP_RUNS", 3, { min: 1 });
	ctx.roleTimeoutSeconds = number("ROLE_TIMEOUT_SECONDS", 0);
	...
	ctx.gateTimeoutSeconds = number("GATE_TIMEOUT_SECONDS", ctx.roleTimeoutSeconds);
	...
	if (ctx.minReviewers > 3) throw new Die("MIN_REVIEWERS cannot exceed the three configured reviewer roles");
	...
	if (env.PIPELINE_UNATTENDED === "1" && !flags.unattended) {
	...
	// Kept, because switching the guard off is the operator's documented
	// choice — but never silent: a user-level PIPELINE_GUARD=off set for
	// interactive sessions also reaches every implementer this run starts.
	if (guardDisabled(env)) warn(`warning: ${GUARD_OFF_WARNING}`);
	if (flags.unattended || flags.autoMerge) {
		if (!flags.assumeYes) {
			...
			const reply = await ask(
				flags.unattended
					? `Run unattended (privileged steps, auto-merge=${flags.autoMerge ? 1 : 0})? [y/N] `
					: "Run with --auto-merge (declares intent for an adapted merge step; grants no privileges)? [y/N] ",
			);
			if (!/^[yY]/.test(reply)) throw new Die("aborted at the safety gate");
		}
	}
	// Trust comes from --unattended alone. --auto-merge used to grant it too,
	// which made a flag named after merging the shortest route to `--approve` /
	// `bypassPermissions` — the one effect its name does not mention, and the
	// opposite of what `run --help` promises for it ("parsed and confirmed, not
	// implemented"). It still passes the startup gate, because an adapted
	// pipeline that hooks a real merge here must stay human-acknowledged.
	if (flags.unattended) {
	...
	ctx.trusted = Boolean(flags.unattended);
	...
	const { errors, warnings } = validate(source.config, source, env);
	...
	// The contract is clean; now the prose. A marker govern left in SOUL.md or
	// a harness copy is an open decision that would enter every prompt as if
	// it were one. A dry-run only notes it: routing and prompts are still worth
	// checking while a decision is pending.
	const markers = scanDecisionMarkers(root, ctx);
	if (markers.length > 0) {
		for (const m of markers) {
			warn(`${flags.dryRun ? "note" : "governance error"}: ${m.file}:${m.line} still carries [${m.marker}]; decide it in /govern before a real run`);
		}
		if (!flags.dryRun) return 2;
	}
	...
	ctx.harnessRel = [...new Set(ctx.preserve.map((p) => relativeOrNull(root, p)).filter(Boolean))];
	ctx.controlPaths = controlPaths(root, ctx.pipelineDir);
	if (ctx.binaryReviewFile) ctx.controlPaths.push(ctx.binaryReviewFile);
	ctx.diffProtectedPaths = [...ctx.integrityPaths, ...ctx.controlPaths];
...
async function mainLoop(ctx) {
	...
	let issues;
	try {
		issues = await guardControl(ctx.controlPaths, ctx.root, "issue source", () => ctx.issueSource.list());
	} catch (error) {
	...
	const openIds = new Set(issues.map((i) => i.raw));
	const states = readAllStates(ctx.pipelineDir);
	for (const [id, state] of Object.entries(states)) if (state.error) throw new Die(`state ${id}: ${state.error}`);
	const ownerOf = (issue) => {
		const id = sanitizeIssueId(issue.raw);
		const owners = Object.entries(states).filter(([, s]) => Object.hasOwn(s.issues, id));
		if (owners.length > 1) throw new Die(`issue ${issue.raw} belongs to multiple state trees; repair the state before resuming`);
		if (owners.length) return owners[0][0];
		let current = issue;
		while (current.parent) {
			const parent = issues.find((i) => i.raw === current.parent);
			if (!parent) return sanitizeIssueId(current.parent);
			current = parent;
		}
		return sanitizeIssueId(current.raw);
	};
	for (const issue of issues) {
		const isChild = Boolean(issue.parent);
		if (flags.onlyIssue) {
			if (issue.raw !== flags.onlyIssue) continue;
		} else if (isChild && openIds.has(issue.parent)) continue;
		matched++;
		if (ctx.haltReason) {
		...
		await processIssue(ctx, issue, { root: ownerOf(issue), depth: 0 });
	}
...
function commitApproval(ctx, args) {
	const result = commitApproved({ root: ctx.root, issueRel: ctx.issueRel, stderr: ctx.stderr, protectedPaths: ctx.diffProtectedPaths, ...args });
	if (!result.committed) {
...
async function runRole(ctx, { root, issueId, role, prompt, outPath, attemptTagText = "" }) {
	...
	const result = await guardControl(ctx.controlPaths, ctx.root, role, () => invokeRole({
	...
	if (result.error) ctx.stderr.write(`warning: ${role} on ${result.harness}: ${result.error}\n`);
		...
		// A reviewer keeps the bytes it managed to emit; they are evidence, and
		// the seat is refused separately. Every other class loses its answer.
		const kept = role.startsWith("review.") ? "its verdict does not count, any finding it already wrote does" : "its answer is discarded";
		ctx.stderr.write(`warning: ${role} timed out after ${ctx.roleTimeoutSeconds}s; ${kept}\n`);
	}
...
function integrityCheck(ctx, before, role, work, attemptNo, issueId, root) {
	...
	if (unrestored.length) throw new Die(`governance restoration incomplete: ${unrestored.map((p) => relative(ctx.root, p)).join(", ")}; run stopped for inspection`);
...
function implementationChanges(ctx, out, issue = null) {
	const coverage = captureDiff({ root: ctx.root, out, maxBytes: ctx.diffMaxBytes, harnessRel: ctx.harnessRel, protectedPaths: ctx.diffProtectedPaths });
	if (issue && coverage.submodules?.length) pauseReview(ctx, issue.root, issue.issueId, coverage, out);
	const paths = readDiffPaths(`${out}.paths`);
	// captureDiff always writes the manifest siblings; this caller only wanted
	// the path list, so it takes all four away again instead of leaving
	// preexisting.patch.entries.json and blocked.patch.binary.json behind.
	for (const suffix of ["", ".paths", ".entries.json", ".binary.json"]) rmSync(`${out}${suffix}`, { force: true });
	return paths;
}
...
function stashBlocked(ctx, { issueId, work }) {
	...
	const stashed = stashRejectedTree({ root: ctx.root, workDir: work, preserve: ctx.preserve, message: `pipeline: blocked ${issueId}-${ctx.runId}`, stderr: ctx.stderr, protectedPaths: ctx.diffProtectedPaths });
	// A refused stash leaves the rejected tree in place. The next issue would
	// review it as its own work and commit it on approval, so the run stops.
	if (!stashed) haltAfterFailedStash(ctx, `after blocking ${issueId}`);
	return stashed;
}

function haltAfterFailedStash(ctx, when) {
	if (ctx.haltReason) return;
	ctx.haltReason = `git stash failed ${when}; the rejected tree is still in the working tree`;
	ctx.haltAdvice = "Restore a clean tree (git stash, or git checkout -- . and git clean) before starting another run.";
	ctx.haltExit = 1;
}

// A gate or an implementer that moves HEAD has committed work nobody
// reviewed. The issue is blocked and the whole run stops; commits and the
// remaining edits stay in place for inspection.
function headMovedDuring(ctx, { headBefore, phase, role, issueId, root }) {
	...
	const reason = `HEAD moved during ${phase} (${headBefore.slice(0, 7)} -> ${headAfter.slice(0, 7) || "missing"}); committed changes have not been reviewed`;
	...
	blockIssue({ pipelineDir: ctx.pipelineDir, root, issueId, reason, memoryFile: ctx.memoryFile, stderr: ctx.stderr });
...
// A reviewer whose process failed did not complete a review. Its approval
// counts for nothing; a blocking finding it did write down is kept, because
// a finding is evidence whatever the exit code. Otherwise the file is emptied
// so the gate treats the reviewer as unavailable (retry, then panel floor).
function demoteFailedReviewer(ctx, { focus, file, result, label }) {
	if (!result.timedOut && result.status === 0) return true;
	if (hasBlockingFindings(ctx, file)) {
		ctx.stderr.write(`reviewer ${focus}${label} process failed; its blocking findings are kept, its verdict is not an approval\n`);
		return false;
	}
	ctx.stderr.write(`reviewer ${focus}${label} process failed; its output is discarded (a failed review cannot approve)\n`);
	writeFileSync(file, "");
	return false;
}

function hasBlockingFindings(ctx, file) {
	return Boolean(extractJson(readText(file))?.findings.some((f) => ctx.blocking.includes(severityOf(f)) || !KNOWN_SEVERITY.has(severityOf(f))));
}
...
async function processIssue(ctx, issue, { root, depth }) {
	...
	if (!flags.dryRun) {
		...
		const saved = loadState(ctx.pipelineDir, root).issues[issueId];
		if (!saved.contributors) {
			setContributors(ctx.pipelineDir, root, issueId, [
				...(ctrlAttempts ? [ctx.models.implement ?? "default"] : []),
				...(masterAttempts ? [ctx.models.implement_master ?? "default"] : []),
			]);
		}
		...
		if (a.status === "paused") {
			const out = join(work, "diff.patch");
			const coverage = captureDiff({ root: ctx.root, out, maxBytes: ctx.diffMaxBytes, harnessRel: ctx.harnessRel, binaryReviews: ctx.binaryReviews, protectedPaths: ctx.diffProtectedPaths });
			if (coverage.truncated.length || coverage.omitted.length) throw new Die(reviewPauseReason(issueId, coverage, out));
			setReviewPause(ctx.pipelineDir, root, issueId, null);
			stderr.write(`review pause resolved for ${issueId}; resuming implementation\n`);
		}
	...
	if (!flags.dryRun && ctrlAttempts + masterAttempts === 0) {
		const paths = implementationChanges(ctx, join(work, "preexisting.patch"), { root, issueId });
		if (paths.length > 0) {
	...
	// Consecutive attempts whose reviewer output could not be fully processed.
	let unprocessedStreak = 0;
	...
	for (;;) {
		...
		// Reserve durably before starting: crashes/timeouts still spend this attempt.
		if (!flags.dryRun) recordAttempt(ctx.pipelineDir, root, issueId, role === "implement" ? "controller" : "master", ctx.models[role] ?? "default");
		...
		if (!flags.dryRun && headMovedDuring(ctx, { headBefore, phase: role, role, issueId, root })) return "failed";
		...
		// whole tree budget.
		if (!flags.dryRun && ctx.gates.length > 0) {
			// The gates run project commands, and the implementer writes the
			// scripts those commands call. So the same two checks as after the
			// implementer, on a fresh snapshot: governance byte-identical, HEAD
			// unmoved. A committed file would otherwise vanish from the review
			// diff, which compares the working tree with HEAD.
			const beforeGates = governanceSnapshot(ctx, work);
			let gateFailed = false;
			for (const gate of ctx.gates) {
				const r = await guardControl(ctx.controlPaths, ctx.root, `gate ${gate.name}`, () => runShell(gate.run, { cwd: ctx.root, env: ctx.childEnv, timeoutMs: ctx.gateTimeoutSeconds * 1000 }));
			...
			const gatesClean = integrityCheck(ctx, beforeGates, "gate", work, attemptNo, issueId, root);
			if (headMovedDuring(ctx, { headBefore, phase: "the gates", role: "gate", issueId, root })) return "failed";
			if (gateFailed || !gatesClean) continue;
		}

		const diffFile = join(work, "diff.patch");
		const coverage = captureDiff({ root: ctx.root, out: diffFile, maxBytes: ctx.diffMaxBytes, harnessRel: ctx.harnessRel, binaryReviews: ctx.binaryReviews, protectedPaths: ctx.diffProtectedPaths });
		if (!flags.dryRun && (coverage.truncated.length || coverage.omitted.length)) {
			pauseReview(ctx, root, issueId, coverage, diffFile);
		}
		...
		const contributors = flags.dryRun ? [implModel] : loadState(ctx.pipelineDir, root).issues[issueId].contributors ?? [implModel];
		const ran = [];
		let unmappedN = 0;
		const launches = [];
		for (const focus of ["security", "quality", "correctness"]) {
			...
			if (ctx.noSelfReview && contributors.some((model) => model !== "default" && modelIdentity(rmodel) === modelIdentity(model))) {
		...
		const launched = settled.map((r) => r.value);
		// `launches` and `ran` are pushed together, so they align by index.
		if (!flags.dryRun) ran.forEach((r, i) => { r.available = demoteFailedReviewer(ctx, { focus: r.focus, file: r.file, result: launched[i], label: "" }); });
		...
		if (flags.dryRun) {
			// These prompts need the panel's verdicts, so a dry-run cannot render
			// them — but their routing is exactly what the operator checks here.
			const later = [
				["controller", "prompt needs the reviewers' verdicts"],
				["master_review", "prompt needs the reviewers' verdicts"],
				...(role === "implement" ? [["implement_master", "runs after max_attempts_controller rejected attempts"]] : []),
			];
			for (const [r, why] of later) stdout.write(`[dry-run] ${r} -> ${ctx.models[r] ?? "default"} (not rendered: ${why})\n`);
		...
		for (const r of ran) {
			...
			if (check.exit === 0 && r.available) continue;
			...
			const retried = await runRole(ctx, {
				root,
				issueId,
				role: `review.${r.focus}`,
			...
			const available = demoteFailedReviewer(ctx, { focus: r.focus, file: retryFile, result: retried, label: " (retry)" });
			// Answer selection controls the panel seat only. Findings from both
			// processes survive even when the retry is malformed or discarded.
			r.evidenceFile = `${r.file}.evidence`;
			writeFileSync(r.evidenceFile, JSON.stringify({ role: r.focus, evidence_only: true, findings: [
				...(extractJson(readText(r.file))?.findings ?? []),
				...(extractJson(readText(retryFile))?.findings ?? []),
			] }));
			const again = existsSync(retryFile) ? checkReviewerText(readText(retryFile)) : { exit: 1, worst: -1 };
			const preservesBlocking = !hasBlockingFindings(ctx, r.file) || hasBlockingFindings(ctx, retryFile);
			if ((rankOfCheck(again.exit) > rankOfCheck(check.exit) || (!r.available && available && again.exit === 0)) && again.worst >= check.worst && preservesBlocking) {
		...
		if (ranN > 0) {
			const g = runGate({ files: ran.map((r) => r.file), evidenceFiles: ran.flatMap((r) => r.evidenceFile ? [r.evidenceFile] : []), unavailableFiles: ran.filter((r) => !r.available).map((r) => r.file), blocking: ctx.blocking, followup: ctx.followup, minReviewers: ctx.minReviewers });
			writeFileSync(gateFile, `${JSON.stringify(g.result, null, 2)}\n`);
			gateStatus = g.exit;
			const concat = Buffer.concat(ran.flatMap((r) => [r.file, ...(r.evidenceFile ? [r.evidenceFile] : [])]).map((file) => existsSync(file) ? readFileSync(file) : Buffer.alloc(0)));
			...
			if (concat.length > ctx.reviewersMaxBytes) {
				const trunc = Buffer.concat([
					sliceUtf8(concat, ctx.reviewersMaxBytes),
		...
		// Reviewer output that could not be fully processed blocks the attempt
		// on purpose: what the scan did not reach may be the critical nobody
		// read, and a clean retry is a different answer, not proof about the
		// first. But it is a format problem, and the implementer cannot fix a
		// reviewer's formatting — left alone it would spend every attempt of
		// the tree on it. Same shape as the two streaks above: the second one
		// in a row ends the issue naming the real cause.
		let unprocessed = false;
		try {
			unprocessed = (JSON.parse(readText(gateFile)).blocking ?? []).some((f) => severityOf(f) === UNPROCESSED_SEVERITY);
		} catch {}
		if (unprocessed) {
			stderr.write(`reviewer output could not be fully processed; this attempt cannot be approved (a severity outside every parseable object, or the JSON scan limit)\n`);
		}
		unprocessedStreak = unprocessed ? unprocessedStreak + 1 : 0;
		if (unprocessedStreak >= 2) {
			const reason = `Configuration error: two consecutive attempts carried reviewer output that could not be fully processed. A severity key sat outside every parseable object — a reviewer quoting JSON out of the diff does this — or the JSON scan limit was reached. It is a reviewer format problem, not a code-quality signal, and no implementation can clear it. Check models.review.* in ${ctx.agentsFile}; a reviewer must answer with one JSON object and must not quote JSON from the diff.`;
			stderr.write(`${reason}\n`);
		...
		const masterRun = await runRole(ctx, {
			root,
			issueId,
			role: "master_review",
		...
		// A master whose process failed did not decide. Its "approve" or
		// "split" is not acted on; a reject or take_over it wrote stands,
		// because neither releases work.
		if ((masterRun.timedOut || masterRun.status !== 0) && (decision === "approve" || decision === "split")) {
			stderr.write(`master review process failed (exit ${masterRun.timedOut ? "timeout" : masterRun.status}); a failed master cannot ${decision} — treating it as reject\n`);
			appendFileSync(exclusionsFile, `--- attempt ${attemptNo} (master process failed) ---\nThe master review process exited abnormally; its decision was not applied.\n\n`);
			decision = "reject";
		}
		...
		if (decision === "approve" && gateStatus === 0) {
			if (ctx.issueSource.kind === "file" && !(await ctx.issueSource.markDone(issueRaw))) throw new Die(`approved issue ${issueRaw} could not be marked done; no commit created`);
			setIssueStatus(ctx.pipelineDir, root, issueId, "done");
			stdout.write(`approved: ${issueId}\n`);
			if (ctx.issueSource.kind !== "file") await ctx.issueSource.markDone(issueRaw);
			recordFollowups(ctx, { issueId, gateFile });
			if (ctx.commitApproved) {
				commitApproval(ctx, { issueId, issueLine: issue.line, pathsFile: `${diffFile}.paths`, reviewed: coverage.reviewed });
			} else {
		...
		if (decision === "take_over") {
			...
			// inherit it.
			const fresh = stashRejectedTree({ root: ctx.root, workDir: work, preserve: ctx.preserve, message: `pipeline: pre-take_over ${issueId}-${ctx.runId}`, stderr, protectedPaths: ctx.diffProtectedPaths });
			if (fresh) setContributors(ctx.pipelineDir, root, issueId, []);
...
// Findings at a follow-up severity do not block the approval, and they are the
// only review output that outlives the issue. gate.json holds them under the
// gitignored `.pipeline/`, which is not where a human looks for what the
// project still owes — so an approval appends them to MEMORY.md as engine
// history, next to blockers and pauses. The text is written by reviewers:
// quoted decision markers are escaped, exactly as in a blocker.
function recordFollowups(ctx, { issueId, gateFile }) {
	const list = followupsToProse(gateFile);
	if (!list) return;
	appendFileSync(
		ctx.memoryFile,
		quoteDecisionMarkers(`\n## Follow-ups — ${issueId} (${isoDate()})\n\nAccepted with the approval; triage into the backlog by hand.\n${list}`),
	);
	ctx.stderr.write(`follow-ups of ${issueId} recorded in ${ctx.memoryFile}\n`);
}

function pauseReview(ctx, root, issueId, coverage, out) {
	const reason = reviewPauseReason(issueId, coverage, out);
	setReviewPause(ctx.pipelineDir, root, issueId, reason);
	appendFileSync(ctx.memoryFile, quoteDecisionMarkers(`\n## Review pause — ${issueId}\n\n${reason}\n`));
	throw new Die(reason);
}

function reviewPauseReason(issueId, coverage, out) {
	const binary = coverage.omitted.filter((p) => Object.hasOwn(coverage.binary ?? {}, p));
	const submodules = coverage.submodules ?? [];
	const text = [...coverage.truncated, ...coverage.omitted.filter((p) => !binary.includes(p) && !submodules.includes(p))];
	return `incomplete review diff for ${issueId}: ${[...text, ...binary, ...submodules].join(", ")}. Paused; work is retained and unchanged restarts spend no implementation budget. ` +
		(text.length ? "Text content: reduce the change or raise DIFF_MAX_BYTES, then resume. " : "") +
		(submodules.length ? "Submodule changes are not supported by automatic review. Review and commit those changes separately, then resume the remaining implementation; increasing DIFF_MAX_BYTES or a binary receipt cannot approve a submodule. " : "") +
		(binary.length ? `Binary content needs human review: inspect the exact Git blobs in ${out}.entries.json, then copy ${out}.binary.json outside the repository, set approved to true and resume with BINARY_REVIEW_FILE pointing to that receipt. See operations.md, Binary review.` : "");
}
...
async function splitIssue(ctx, { issue, issueId, issueRaw, root, depth, work, subIssues, findingsFile, exclusionsFile, attemptNo }) {
	...
	// A refused stash halts before the first child: it would otherwise review
	// the parent's rejected tree as its own diff.
	if (!stashRejectedTree({ root: ctx.root, workDir: work, preserve: ctx.preserve, message: `pipeline: pre-split ${issueId}-${ctx.runId}`, stderr, protectedPaths: ctx.diffProtectedPaths })) {
		haltAfterFailedStash(ctx, `after splitting ${issueId}`);
	}
...
async function openChildren(ctx, { issueRaw, root }) {
	const list = await guardControl(ctx.controlPaths, ctx.root, "split issue source", () => ctx.issueSource.list());
	return list
...
async function finishChildren(ctx, { issue, issueId, issueRaw, root, depth, work, children }) {
	...
	// --max-runs is a planned stop, not a failed child: the parent keeps its
	// `split` status so the next run resumes at the open children.
	let paused = false;
	for (const child of children) {
		...
		if (["skipped", "paused"].includes(await processIssue(ctx, child, { root, depth: depth + 1 }))) paused = true;
	}
	if (ctx.haltReason) {
	...
	if (paused) {
		stderr.write(`split ${issueId} left open: --max-runs ${ctx.maxRuns} reached before every sub-issue finished; the next run resumes at the open sub-issues\n`);
		return "paused";
	}
