...
import { KNOWN_SEVERITY, checkReviewerText, extractJson, severityOf } from "../review/reviewer-output.mjs";
...
import { attemptTag, excerpt, headTail, intEnv, isoDate, runIdNow, sanitizeIssueId, tailLines } from "../util/text.mjs";
...
function stashBlocked(ctx, { issueId, work }) {
	...
	const stashed = stashRejectedTree({ root: ctx.root, workDir: work, preserve: ctx.preserve, message: `pipeline: blocked ${issueId}-${ctx.runId}`, stderr: ctx.stderr });
...
async function processIssue(ctx, issue, { root, depth }) {
	...
	for (;;) {
		...
		if (decision === "take_over") {
			...
			// inherit it.
			const fresh = stashRejectedTree({ root: ctx.root, workDir: work, preserve: ctx.preserve, message: `pipeline: pre-take_over ${issueId}-${ctx.runId}`, stderr });
...
async function splitIssue(ctx, { issue, issueId, issueRaw, root, depth, work, subIssues, findingsFile, exclusionsFile, attemptNo }) {
	...
	if (!stashRejectedTree({ root: ctx.root, workDir: work, preserve: ctx.preserve, message: `pipeline: pre-split ${issueId}-${ctx.runId}`, stderr })) {
