...
async function run({ root, flags, env, stdout, stderr }) {
	...
	ctx.diffMaxBytes = intEnv(env, "DIFF_MAX_BYTES", 524288);
	ctx.binaryReviews = {};
...
async function processIssue(ctx, issue, { root, depth }) {
	...
	if (!flags.dryRun) {
		...
		if (a.status === "paused") {
			...
			const coverage = await guardControl(ctx.controlPaths, ctx.root, "paused diff capture", () => captureDiff({ root: ctx.root, out, maxBytes: ctx.diffMaxBytes, harnessRel: ctx.harnessRel, binaryReviews: ctx.binaryReviews }));
	...
	for (;;) {
		...
		const coverage = await guardControl(ctx.controlPaths, ctx.root, "review diff capture", () => captureDiff({ root: ctx.root, out: diffFile, maxBytes: ctx.diffMaxBytes, harnessRel: ctx.harnessRel, binaryReviews: ctx.binaryReviews }));
		if (!flags.dryRun && (coverage.truncated.length || coverage.omitted.length)) {
			const reason = reviewPauseReason(issueId, coverage, diffFile);
			setReviewPause(ctx.pipelineDir, root, issueId, reason);
			appendFileSync(ctx.memoryFile, `\n## Review pause — ${issueId}\n\n${reason}\n`);
		...
		if (ranN > 0) {
			const g = runGate({ files: ran.map((r) => r.file), unavailableFiles: ran.filter((r) => !r.available).map((r) => r.file), blocking: ctx.blocking, followup: ctx.followup, minReviewers: ctx.minReviewers });
...
function reviewPauseReason(issueId, coverage, out) {
	...
	const text = [...coverage.truncated, ...coverage.omitted.filter((p) => !binary.includes(p))];
	return `incomplete review diff for ${issueId}: ${[...text, ...binary].join(", ")}. Paused; work is retained and unchanged restarts spend no implementation budget. ` +
