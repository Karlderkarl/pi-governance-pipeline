...
import { REVIEWERS, ROLES, eachMappedRole, modelIdentity, modelRef, thinkingRef, THINKING_LEVELS, vendorOf } from "./parse.mjs";
...
// A known field with the wrong type is a mistake, not absence: `implement:
// sonnet` would otherwise run the default model, and `no_self_review: "true"`
// would count as on for the validator and off for the loop (which compares
// `=== true`). Checked on the parsed YAML before defaults are merged in, so a
// value that exists but is unusable cannot masquerade as a missing one.
function typeErrors(raw, errors) {
	if (!isMap(raw)) return;
	const bad = (field, expected, v) => errors.push(`AGENTS.md ${field} must be ${expected}; got ${JSON.stringify(v)}`);
	if (present(raw.models) && !isMap(raw.models)) return bad("models", "a map of roles", raw.models);
	const m = raw.models ?? {};
	for (const role of ROLES) {
		if (present(m[role]) && !isMap(m[role])) bad(`models.${role}`, "a map { provider: <provider>, model: <id> }", m[role]);
	}
	if (present(m.review) && !isMap(m.review)) bad("models.review", "a map of reviewer roles", m.review);
	else {
		for (const r of REVIEWERS) {
			if (present(m.review?.[r]) && !isMap(m.review[r])) bad(`models.review.${r}`, "a map { provider: <provider>, model: <id> }", m.review[r]);
		}
	}
	if (present(m.constraints) && !isMap(m.constraints)) bad("models.constraints", "a map", m.constraints);
	else if (present(m.constraints?.no_self_review) && typeof m.constraints.no_self_review !== "boolean") {
		bad("models.constraints.no_self_review", "true or false, unquoted", m.constraints.no_self_review);
	}
	if (present(raw.budgets) && !isMap(raw.budgets)) bad("budgets", "a map of integer fields", raw.budgets);
	if (present(raw.review) && !isMap(raw.review)) bad("review", "a map with blocking_severities and followup_severities", raw.review);
}

export function validate(config, source = {}, env = process.env) {
	const errors = [];
	const warnings = [];
	...
	typeErrors(source.raw ?? config, errors);

	const impl = modelRef(m.implement);
	const master = modelRef(m.implement_master);
	// Compared by the model they name: an aggregator prefix or a thinking
	// level does not make a second model.
	const same = (a, b) => Boolean(a && b) && modelIdentity(a) === modelIdentity(b);
	if (same(impl, master)) {
	...
	for (const r of REVIEWERS) {
		...
		// The vendor, not the route: openrouter/google/x and openrouter/anthropic/y
		// are two providers' models behind one API.
		providers.add(vendorOf(modelRef(entry)) ?? provider);
	}
	...
	if (splitOk && b.max_split_depth > 1 && env.PIPELINE_ALLOW_DEEP_SPLIT !== "1") {
		errors.push(
	...
	eachMappedRole(m, (path, entry) => {
		...
		// Presence is not enough. modelRef() interpolates these two fields into
		// `provider/model`, so a nested map or a number reaches the harness as
		// `anthropic/[object Object]` — routing that validates, passes doctor,
		// and only fails at the first model call, two attempts of tree budget
		// later. The reviewer roles have required a string provider since 1.2.0;
		// every other role was left out. An unquoted `model: 4.5` lands here too
		// (YAML reads it as a float), which is why the message says to quote it.
		for (const [field, value] of [["model", entry.model], ["provider", entry.provider]]) {
			if (field === "provider" && value === undefined) continue;
			if (typeof value !== "string" || value.trim() === "") {
				errors.push(
					`AGENTS.md models.${path}.${field} must be a non-empty string; got ${JSON.stringify(value)} — quote the value if it looks like a number`,
				);
			}
		}
	...
	if (noSelfReview) {
		...
		for (const [label, implModel] of [
			...
			const collisions = REVIEWERS.filter((r) => same(modelRef(m.review?.[r]), implModel)).length;
