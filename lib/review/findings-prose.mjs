...
import { UNPROCESSED_SEVERITY } from "./reviewer-output.mjs";
...
// The unprocessed-review marker blocks the attempt, but it is a message to
// the reviewer about its own output format. Prose from this file goes into the
// implement prompt, where "Return complete review JSON" is an instruction the
// implementer cannot act on — and, repeated over every attempt, is the only
// thing it would be told. The gate JSON and the run log keep it.
const isMarker = (f) => String(f?.severity ?? "").trim().toLowerCase() === UNPROCESSED_SEVERITY;
...
export function findingsToProse(gate) {
	const g = readGate(gate);
	if (!g) return "";
	const blocking = (g.blocking || []).filter((f) => !isMarker(f));
	const followups = (g.followups || []).filter((f) => !isMarker(f));
