...
export function extractJson(text) {
	...
	for (const candidate of jsonCandidates(text)) {
		...
		if (Array.isArray(parsed.findings)) {
			parsed.invalidFindings = parsed.findings.some((f) => !f || typeof f !== "object" || Array.isArray(f));
	...
	return selected ? { ...selected, findings: [...new Map(evidence.map((f) => [JSON.stringify(f), f])).values()], invalidFindings: invalid } : null;
}
