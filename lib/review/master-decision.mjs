...
import { hasUnprocessedJsonKey, jsonCandidates } from "./reviewer-output.mjs";
...
export function parseMasterDecision(text) {
	// Same fail-closed half as the reviewer parser: a `"decision"` the scanner
	// never reached may be the strictest one in the output, so the two
	// consequential verdicts do not stand while one is unaccounted for.
	const unreached = hasUnprocessedJsonKey(text ?? "", "decision", (object) => {
		const decision = String(object.decision ?? "").trim().toLowerCase();
		if (decision.includes("|")) return null;
		return new Set(Object.hasOwn(RANK, decision) || decision === "split" ? [object] : []);
	});
	let d = null;
	let reasons = [];
	let splitObj = null;
	...
	if (unreached && (d === null || d === "approve" || splitObj)) {
		return {
			decision: d === "approve" || d === null ? "reject" : d,
			reasons,
			split: null,
			note: "a decision was not read or the JSON scan limit was reached; approve and split do not stand while the output is unaccounted for",
		};
	}
	if (splitObj) {
