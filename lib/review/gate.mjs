...
export function runGate({ files, blocking = ["critical", "high"], followup = ["medium", "low"], minReviewers = 1, unavailableFiles = [] }) {
	const unavailable = [];
	const findings = [];
	const seen = new Map();
	...
	for (const file of files) {
		...
		if (parsed.invalidFindings || unavailableFiles.includes(file)) unavailable.push(file);
