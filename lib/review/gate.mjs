...
export function runGate({ files, blocking = ["critical", "high"], followup = ["medium", "low"], minReviewers = 1, unavailableFiles = [], evidenceFiles = [] }) {
	const unavailable = [];
	const findings = [];
	const seen = new Map();
	const blocks = (f) => blocking.includes(severityOf(f)) || !KNOWN_SEVERITY.has(severityOf(f)) || !followup.includes(severityOf(f));

	for (const file of [...files, ...evidenceFiles]) {
		const evidenceOnly = evidenceFiles.includes(file);
		let parsed = null;
		try {
		...
		if (!parsed || !Array.isArray(parsed.findings)) {
			if (!evidenceOnly) unavailable.push(file);
			continue;
		}
		const role = parsed.role ?? file;
		// Evidence survives a failed process/schema, but it never fills a panel seat.
		if (!evidenceOnly && (parsed.invalidFindings || unavailableFiles.includes(file))) unavailable.push(file);
		for (const finding of parsed.findings) {
			...
			if (prev) {
				...
				if (blocks(prev) !== blocks(next) ? blocks(prev) : rankOf(prev) >= rankOf(next)) continue;
