...
// PIPELINE_GUARD=off switches the whole extension off. It is often set once,
// user-wide, for interactive sessions — and then also reaches every
// implementer a pipeline run starts. doctor and run name it; neither drops it.
export const guardDisabled = (env = process.env) => env.PIPELINE_GUARD === "off";
export const GUARD_OFF_WARNING =
	"PIPELINE_GUARD=off is set; pipeline-guard is disabled in this session and in every implementer process — governance writes, sudo, recursive deletes and force-pushes are not gated (only the engine's snapshot checks remain)";
...
export function controlPath(path) {
	...
	return /(?:^|\/)\.git(?:\/|$)/i.test(p) || /(?:^|\/)\.pipeline\/state(?:\/|$)/i.test(p);
}
