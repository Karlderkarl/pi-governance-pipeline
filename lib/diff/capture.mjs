...
export function captureDiff({ root, out, maxBytes, harnessRel = [], binaryReviews = {} }) {
	writeFileSync(out, "");
	...
	const reviewed = withIndex(root, ({ command }) => {
		...
		const selected = rows.map((row) => {
			...
			const body = e.mode === "0" ? Buffer.alloc(0) : command(["cat-file", "blob", e.oid]);
			const patch = command(["diff", "--cached", "--no-ext-diff", "--no-textconv", "--no-renames", ...(baseHead ? [baseHead] : []), "--", `:(literal)${row.path}`]);
	...
	if (n === 0 || chunks.length === 0) {
		writeFileSync(out, "");
		return { included, omitted, truncated, binary, reviewed, empty: true };
	}
	...
	return { included, omitted, truncated, binary, reviewed, empty: false };
}
