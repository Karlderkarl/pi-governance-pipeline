...
export async function invokeRole({ spec, role, model, promptText, outPath, cwd, trusted, timeoutMs, env }) {
	...
	if (result.timedOut) {
		// A timed-out process did not complete its answer, and `timedOut` says
		// so to every caller — the seat it held is marked unavailable there,
		// not here by emptying the file.
		//
		// For a reviewer the bytes it did emit are evidence. A reviewer that
		// wrote a critical finding and then hung must not lose it: the same
		// rule the non-zero-exit path already states ("a finding is evidence
		// whatever the exit code") could never fire for a timeout while this
		// function discarded the output first. Truncated JSON is no risk —
		// extractJson returns null for anything that does not parse.
		//
		// Every other class keeps the empty file. A judge's half-written
		// verdict is a truncated verdict, not evidence, and a truncated
		// research note would be cached for the rest of the issue.
		writeFileSync(outPath, isolation === "reviewer" ? adapter.parseOutput(result).text : "");
