...
export function buildArgs({ isolation, model, trusted }) {
	...
	// `--tools <tools...>` is variadic: it consumes every following argument
	// until the next flag. The tool list is therefore collected here and
	// appended last, deliberately, rather than relying on it happening to be
	// written last in each branch — a flag added after it would silently be
	// read as a tool name.
	let tools = null;
	switch (isolation) {
		case "reviewer":
			args.push("--safe-mode", "--permission-mode", "dontAsk");
