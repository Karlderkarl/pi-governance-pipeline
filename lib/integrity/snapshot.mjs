...
import { closeSync, copyFileSync, existsSync, lstatSync, mkdirSync, openSync, readdirSync, readFileSync, readlinkSync, readSync, realpathSync, rmSync, rmdirSync, statSync, symlinkSync, writeFileSync } from "node:fs";
...
function walk(abs, into, opts) {
	...
	const buf = !opts.hashOnly && st.size <= opts.keepBytes ? readFileSync(abs) : null;
	const entry = { type: "file", hash: buf ? createHash("sha256").update(buf).digest("hex") : hashFile(abs), size: buf?.length ?? st.size, content: null, spill: null };
	if (!opts.hashOnly) {
		if (buf) entry.content = buf;
...
export function restoreSnapshot(before, diff) {
	...
	for (const p of [...diff.changed, ...diff.removed].sort((a, b) => a.length - b.length)) {
		...
		try {
			...
			else if (e.spill && existsSync(e.spill)) {
				if (!lstatSync(e.spill).isFile() || hashFile(e.spill) !== e.hash) throw new Error("snapshot recovery copy changed");
				copyFileSync(e.spill, p);
			}
