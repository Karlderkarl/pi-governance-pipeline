...
// Cut a buffer at `maxBytes` without splitting a UTF-8 sequence. A byte-exact
// cut turns the tail of a multi-byte character into U+FFFD, and these buffers
// go straight into a model's prompt — a reviewer should not have to judge
// mojibake it did not write. Walks back over continuation bytes (0b10xxxxxx)
// to the last complete character.
export function sliceUtf8(buf, maxBytes) {
...
export function intEnv(env, name, fallback, { min = 0, warn = (message) => process.stderr.write(`${message}\n`) } = {}) {
	const raw = env[name];
	if (raw === undefined || raw === "") return fallback;
	const n = Number(raw);
	if (!/^[0-9]+$/.test(raw) || !Number.isSafeInteger(n) || n < min) {
		warn(`warning: ${name}=${JSON.stringify(raw)} must be a decimal safe integer >= ${min}; using default ${fallback}`);
		return fallback;
	}
	return n;
}
