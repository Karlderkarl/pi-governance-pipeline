...
import { wrapperEol, wrapperPin } from "./wrapper.mjs";
import { runLockPath } from "../state/lock.mjs";
...
export function runDoctor({ root, env = process.env, harness = null }) {
	...
	if (isGitWorkTree(root)) {
		pass("git repository");
		const lock = runLockPath(root);
		if (existsSync(lock)) fail(`working tree lock exists: ${lock}; stop surviving pipeline processes and inspect the tree before removing a stale lock`);
	...
	// Markers outside the contract block: govern leaves them wherever a decision
	// is open, and a real run refuses on them. The contract's own markers are
	// already FAILs above; this covers SOUL.md, MEMORY.md and the harness copies.
	const markerFiles = {
		agentsFile,
		soulFile: env.SOUL_FILE ? resolve(root, env.SOUL_FILE) : join(root, "SOUL.md"),
		memoryFile: env.MEMORY_FILE ? resolve(root, env.MEMORY_FILE) : join(root, "MEMORY.md"),
	};
	for (const m of scanDecisionMarkers(root, markerFiles)) fail(`governance: ${m.file}:${m.line} still carries [${m.marker}]; decide it in /govern (a real run refuses to start)`);
	...
	const appendSystem = join(root, ".pi", "APPEND_SYSTEM.md");
	if (existsSync(rootSystem) && !existsSync(appendSystem)) {
		warn("SYSTEM.md exists at the root but .pi/APPEND_SYSTEM.md does not; pi does not load a root SYSTEM.md");
	} else if (existsSync(rootSystem)) {
		// The root file is the reviewed source, the .pi copy is what pi loads.
		// A paraphrase drifts silently: the next edit lands in one of them only.
		const normalized = (path) => readFileSync(path, "utf8").replace(/\r\n/g, "\n");
		if (normalized(rootSystem) !== normalized(appendSystem)) {
			warn("SYSTEM.md and .pi/APPEND_SYSTEM.md differ; pi loads only the .pi copy — copy the root file over it (`cp SYSTEM.md .pi/APPEND_SYSTEM.md`) after reviewing the difference");
		}
	}
	if (guardDisabled(env)) warn(GUARD_OFF_WARNING);
	...
	if (!existsSync(wrapper)) warn("auto-develop.sh missing; run `pipeline init`");
		...
		if (isGitWorkTree(root)) {
			// What git resolves from the repository alone, not what a line in
			// .gitattributes looks like: `linguist-generated=true` reads like a
			// rule and pins nothing, `-text eol=lf` normalises nothing, and a
			// rule in the user's core.attributesFile does not travel with a clone.
			const eol = wrapperEol(root);
			if (!eol.projectRule) warn("auto-develop.sh has no final LF rule in the project's .gitattributes; private Git attributes do not travel with a clone — run `pipeline init`");
			else if (eol.lf) pass("auto-develop.sh has a project LF rule and resolves to text enabled, eol=lf");
			else warn(`auto-develop.sh resolves to text=${eol.text} eol=${eol.eol} in this repository; a checkout can bring it back as CRLF and bash would look for \`bash\\r\` — run \`pipeline init\``);
	...
	for (const [rootId, s] of Object.entries(states)) {
		if (s.error) { fail(`state ${rootId}: ${s.error}`); continue; }
