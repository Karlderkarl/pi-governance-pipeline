...
import { hasWrapperEolRule, wrapperEol, wrapperEolRule, wrapperPin, wrapperText } from "./wrapper.mjs";
...
export async function initCommand(argv, { root = process.cwd(), env = process.env } = {}) {
	...
	// Wrapper, before any other setup file. Success means the wrapper in place
	// is the one this package generates: the automate mode runs
	// `./auto-develop.sh --dry-run` next, and a foreign script or another pin
	// would run instead of the engine that just validated the contract.
	...
	const existing = existsSync(wrapper) && !force ? readFileSync(wrapper, "utf8") : null;
	if (existing !== null) {
		const pin = wrapperPin(existing);
		let refusal = null;
		if ((harness !== null || local) && existing.replace(/\r\n/g, "\n") !== text) refusal = "requested configuration was not applied";
		else if (!pin) refusal = "exists and is not a generated wrapper";
		else if (pin !== version) refusal = `pinned to ${pin}, this package is ${version}`;
		if (refusal) {
			out(`auto-develop.sh: ${refusal}; pass --force to replace the existing wrapper`);
			out("init: nothing written — the existing wrapper is not this package's pipeline; do not run it as the dry-run");
			return 1;
		}
		out(`auto-develop.sh: already pinned to ${version}`);
	} else {
	...
	// Ask git what it resolves, not what a line looks like. Any line starting
	// with `auto-develop.sh ` used to count as a line-ending rule, so
	// `linguist-generated=true` pinned nothing, and `text eol=crlf` or `-text`
	// pinned the opposite — while init reported success either way. Attributes
	// can also come from a deeper .gitattributes or the user's global file,
	// which no test on this one file could ever see.
	const attributes = join(root, ".gitattributes");
	const attrLine = wrapperEolRule();
	const inGit = isGitWorkTree(root);
	const attrText = existsSync(attributes) ? readFileSync(attributes, "utf8") : "";
	const hadOwnRule = attrText.split(/\r?\n/).some((l) => /^auto-develop\.sh\s/.test(l.trim()));
	const hadProjectRule = hasWrapperEolRule(root);
	if (!hadProjectRule) {
		appendFileSync(attributes, `${attrText === "" || attrText.endsWith("\n") ? "" : "\n"}${attrLine}\n`);
	}
	// Outside Git the durable rule is sufficient for setup; the missing Git
	// prerequisite is reported below. Inside Git, verify private overrides too.
	const eol = inGit ? wrapperEol(root) : null;
	if (eol && !eol.lf) {
		fails++;
		out(`FAIL .gitattributes: the project has \`${attrLine}\`, but git resolves text=${eol.text} eol=${eol.eol} for auto-develop.sh; a private attributes override prevents LF normalisation — the wrapper can come back as CRLF and fail with \`bash\\r\``);
	} else if (hadProjectRule) {
		out(".gitattributes: auto-develop.sh already has the project's LF rule");
	} else {
		out(`.gitattributes: pinned auto-develop.sh to LF${hadOwnRule ? " (a weaker or conflicting rule for it was already there; the appended line wins)" : ""}`);
	}
